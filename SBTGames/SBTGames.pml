<?xml version="1.0" encoding="UTF-8" ?>
<Package name="SBTGames" format_version="4">
    <Manifest src="manifest.xml" />
    <BehaviorDescriptions>
        <BehaviorDescription name="behavior" src="main" xar="behavior.xar" />
    </BehaviorDescriptions>
    <Dialogs />
    <Resources>
        <File name="" src=".DS_Store" />
        <File name="" src="html/.DS_Store" />
        <File name="MavenPro" src="html/Maven_Pro/MavenPro.woff2" />
        <File name="OFL" src="html/Maven_Pro/OFL.txt" />
        <File name="style" src="html/css/style.css" />
        <File name="tickTackToe" src="html/css/tickTackToe.css" />
        <File name="circle" src="html/img/circle.png" />
        <File name="cross" src="html/img/cross.png" />
        <File name="index" src="html/index.html" />
        <File name="app" src="html/js/app.js" />
        <File name="jquery-2.1.4.min" src="html/js/jquery-2.1.4.min.js" />
        <File name="jquery.qimhelpers" src="html/js/jquery.qimhelpers.js" />
        <File name="tickTackToe" src="html/js/tickTackToe.js" />
        <File name="tick-tack-toe" src="html/tick-tack-toe.html" />
        <File name="janken" src="html/css/janken.css" />
        <File name="paper" src="html/img/paper.png" />
        <File name="rock" src="html/img/rock.png" />
        <File name="scissors" src="html/img/scissors.png" />
        <File name="janken" src="html/janken.html" />
        <File name="janken" src="html/js/janken.js" />
    </Resources>
    <Topics />
    <IgnoredPaths />
</Package>
